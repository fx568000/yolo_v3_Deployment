...
def detections_boxes(detections):
    ...
    boxes = tf.concat([x0, y0, x1, y1], axis=-1) # 拼接张量，并以最后维度拼接
...
def letter_box_image(image: Image.Image, input_height: int, input_width: int, output_height: int, output_width: int, fill_value)-> np.ndarray:
    """
    ...
    height_ratio = float(output_height)/input_height
    width_ratio = float(output_width)/input_width
    fit_ratio = min(width_ratio, height_ratio)
    # fit_height = int(image.size[1] * fit_ratio)
    # fit_width = int(image.size[0] * fit_ratio)
    fit_height = int(input_height * fit_ratio)
    fit_width = int(input_width * fit_ratio)
    ...
    if isinstance(fill_value, int):
        fill_value = np.full(fit_image.shape[2], fill_value, fit_image.dtype) # 空区域填充

    to_return = np.tile(fill_value, (output_height, output_width, 1)) # 填充范围方向确认
    pad_top = int(0.5 * (output_height - fit_height)) # 图像填充区间大小确认
    pad_left = int(0.5 * (output_width - fit_width))
    to_return[pad_top:pad_top+fit_height, pad_left:pad_left+fit_width] = fit_image # 图像填充拼接
    return to_return
